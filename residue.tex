...
%\includegraphics{up}
$$ \int_{-\infty}^{\infty} \frac{e^{iz}dx}{(z-i)(z+i)} = \oint_{C_0} \frac{e^{iz}dx}{(z-i)(z+i)} = \oint_C \frac{e^{iz}dx}{(z-i)(z+i)} = 2\pi i\res_{z=i}\frac{e^{iz}}{(z-i)(z+i)} = \frac{\cancel{2i}\pi e^{i^2}}{\cancel{2i}}=\pi e^{-1}$$
$$ \int_{-\infty}^{\infty} \frac{\cos(x)dx}{z^2+1} =\Re\left[\oint_C \frac{e^{iz}dx}{(z-i)(z+i)}\right] =\Re\left[\frac\pi e\right] =\frac\pi e$$
